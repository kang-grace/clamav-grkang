...
int get_clamd_response(int sockd, const char *filename, struct mem_info *info)
{
    //scantype
    int scantype;
    int *printok = 0;
    ...
    return dsresult(sockd, scantype, filename, printok, info->errors, clamdopts);
}
...
int scanfile(const char *filename, scanmem_data *scan_data, struct mem_info *info)
{
    ...
    if (info->d) { //clamdscan
        if ((sock = connect_clamd(clamdopts)) < 0) {
            info->errors++;
            return -1;
        }
        if (get_clamd_response(sock, filename, info) > 0) {
            info->ifiles++;
            ret = CL_VIRUS;
        }
    } else { //clamscan
